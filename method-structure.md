...
与 [SKILL.md](SKILL.md)、[comments.md](comments.md) 配套。

## 步骤分块标杆（通用模板）

业务方法体推荐按**步骤流水线**组织，格式如下（与具体业务无关）：

- 方法 Javadoc：整段职责 + `@param` / `@return`
...
**不合格**：整段代码无空行、无步骤注释、只有方法 Javadoc 没有体内分块。
...
| 超过 50 行 | 必须步骤分块；或抽 `validate*` / `build*` |
| 超过 80 行且无分块 | 不合格 |
...
私有方法抽出后，公开方法仍保留步骤分块；私有方法体内同样按块分层。
...
| 方法 | 多行 Javadoc + `@param` / `@return` |
| 步骤块 | `// 动词+对象+边界`；块间空一行 |
| 块内 | 复杂分支、口径补单行 `//` |

完整示例见 [comments.md](comments.md)、[examples.md](examples.md)。

## 常见陷阱

| 陷阱 | 对策 |
|------|------|
| 只有 Javadoc、体内无分块 | 补步骤注释与块间空行 |
