...
完整规范与润色示例见 [comments.md](comments.md)。

## 六、方法结构

| 陷阱 | 后果 | 对策 |
|------|------|------|
| 只有 Javadoc、体内无步骤分块 | 难跟流程 | 每步 `//` 总注释 + 块间空行（见 method-structure.md 模板） |
| 步骤注释只有名词无边界 | 看不懂跳过条件 | 写「不通过则不执行」「无数据则清空」等 |
| 块间无空行 | 视觉拥挤 | 每步后空一行 |
| 公开方法 80+ 行无分块 | 难读、难审 | 步骤分块或抽 `validate*` / `build*` |
| 方法注释用单行 `/** ... */` | 与项目规范不一致 | 改标准多行 Javadoc + `@param` |
...
- 每个方法都加千篇一律的 Javadoc（公开业务方法、口径不直观的私有方法才需要）
...
- [ ] 方法 Javadoc 含职责、边界、`@param`/`@return`（非单行 `/** ... */`）
- [ ] 方法体已步骤分块；块间空一行；步骤注释含边界
- [ ] 复杂分支/口径有块内单行注释
- [ ] 无 AI 套话、机翻味、复述代码
- [ ] 中英排版正确；与邻类注释风格一致
